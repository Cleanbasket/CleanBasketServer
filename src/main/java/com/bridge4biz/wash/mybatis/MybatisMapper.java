...
//	@Select("SELECT @X:=@X+1 AS rownum, A.uid, B.address, A.email, A.phone, avg as avgPrice, sum as accruePrice, count, A.rdate FROM (SELECT @X:=0) R, user A INNER JOIN (SELECT uid, CONCAT(address, ' ', addr_number, ' ', addr_building, ' ', addr_remainder) as address, AVG(price) as avg, SUM(price) as sum, COUNT(*) as count FROM orders GROUP BY uid) B ON A.uid = B.uid WHERE authority = 'ROLE_MEMBER' ORDER BY count DESC")
	...
	@Select("SELECT oid, uid, order_number, state, phone, address, addr_number, addr_building, addr_remainder, note, memo, price, dropoff_price, pickup_date, dropoff_date, payment_method, rdate FROM orders WHERE (state = 1 AND date(pickup_date) == date(now())) OR (state = 2 AND date(pickup_date) == date(now())) ORDER BY state ASC, pickup_date ASC, oid ASC")
	ArrayList<DelivererWork> getTodayPickupRequest();

	//오늘의 모든 배달정보를 get --> 매니저용 
	@Select("SELECT oid, uid, order_number, state, phone, address, addr_number, addr_building, addr_remainder, note, memo, price, dropoff_price, pickup_date, dropoff_date, payment_method, rdate FROM orders WHERE (state = 3 AND date(dropoff_date) == date(now())) OR (state = 4 AND date(dropoff_date) == date(now())) ORDER BY state ASC, dropoff_date ASC, oid ASC")
